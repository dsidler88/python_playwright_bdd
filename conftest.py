...
#from pytest_bdd import AllureStepHook
...
def page_auto(request):
    ...
    with sync_playwright() as play:
        ...
        #     browser = play.chromium.launch(headless=False, args=['--no-sandbox'])
        # else:
...
def pytest_runtest_makereport(item):
    ...
    for type in types:
        ...
        if marker_priority:
            item.user_properties.append(("Test Type", marker_priority.name))
...
# allure_step_hook = AllureStepHook()

# @pytest.fixture(autouse=True)
# def register_allure_step_hook():
#     pytest.hook.pytest_sessionstart.call_for_session(allure_step_hook=allure_step_hook)
